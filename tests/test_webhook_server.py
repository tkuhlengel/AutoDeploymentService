...
class TestIntegration:
    ...
    def test_full_webhook_flow_with_real_repo(self, mock_update, client, temp_repo):
        ...
        with patch.object(webhook_server, 'REPO_PATH', temp_repo), \
             patch.object(webhook_server, 'WEBHOOK_SECRET', 'test_secret'):
            ...
            subprocess.run(['git', 'remote', 'add', 'origin', temp_repo], cwd=temp_repo, capture_output=True)
            subprocess.run(['git', 'fetch', 'origin'], cwd=temp_repo, capture_output=True)
            subprocess.run(['git', 'branch', '--set-upstream-to=origin/' + current_branch], cwd=temp_repo, capture_output=True)
